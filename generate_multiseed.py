...
def main(args):
    ...
    if args.prefix_size > 0 and args.prefix_string is not None:
        raise ValueError("prefix_size and prefix_string cannot be set simultaneously")

    if args.prefix_string is not None:
        tokenized_smiles = smiles_utils.tokenize_smiles(args.prefix_string)
        decoding_prefix = [tgt_dict.indices[e] for e in tokenized_smiles.split()]
        if args.tag_prefix_token:
            ss = tgt_dict.indices['[start-of-condition]']
            ee = tgt_dict.indices['[cond-generation]']
            decoding_prefix.append(ee)
            decoding_prefix = [ss] + decoding_prefix
            print(decoding_prefix)
